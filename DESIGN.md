...
The `/about/` template keeps the standard title band and a compact shared content container. Its native `about-story` pattern contains a single-column introduction of at most two paragraphs focused on Holiday Kenya Safaris, with the Ashford ownership and 20+ years of experience included discreetly. Omit a separate introductory headline or catalogue CTA. Use 24px top and 20px bottom padding, a 12px paragraph gap, and 1.6 line height. Below it, place **Visit Us** and **Contact Us** in two columns with 24px vertical padding, a 32px column gap, and 12px heading/detail spacing. Stack below 768px with a 24px gap. Montserrat H2s use the standard 20–26px section-title token and stay smaller than the H1. Visit Us presents working hours, address, then a smaller titled, lazy-loaded map and directions link; the iframe height is 220px on desktop and 200px on mobile. Contact Us presents the phone number with a **Chat on WhatsApp** button, then “Keep up with Holiday Kenya Safaris updates on social media” and Facebook and Instagram follow buttons. Use the existing public phone, WhatsApp, and Social links settings; omit the email row, blank optional settings, and entirely empty sections. Retain accessible link labels, visible focus, and 44px targets.

At the client's request, all three compact About contact/social buttons use platform branding: WhatsApp Green (`#25D366`) with its recognizable logo and Midnight Navy text/icon for contrast, Facebook blue with a white `f` logo, and an Instagram purple/pink gradient with a white camera logo. Keep descriptive labels and accessible contrast alongside the icons. This Instagram button is a narrow exception to the site's general no-gradient styling rule; it does not change the Wayfinder palette or other components.
