...
- **Advertorial / Conversion Story:** emotionally focused but evidence-led. Beneath the hero excerpt, show a fully expanded **What we’ll cover** outline of every body H2 and H3, with H3 links nested beneath their preceding H2. Keep it in normal flow as simple anchor links with no sticky or active-section behaviour, then place the early **Request a quote** action. Use a 680–720px reading column beside an approximately 320px sticky Primary Tour panel on desktop, and a mobile quote bar only after the opening action leaves view. Every quote action opens the shared intake and review flow.
...
Advertorials require a published Primary Tour. Their hero outline is generated from the rendered article, preserves usable existing heading IDs, and assigns deterministic unique IDs to every body H2 and H3 that needs one. On desktop, the breadcrumbs, public context and H1 form a full-width first row; the excerpt, outline and opening quote action sit below in the left column beside a fixed 4:3 featured image. The image remains hidden in the Advertorial hero on mobile. The desktop Tour panel may surface only verified canonical facts and a positive stored starting price when available. Repeated quote controls are proxies to one shared intake dialog; do not duplicate forms. On mobile, use `IntersectionObserver` or an equivalent visibility primitive to reveal the sticky quote bar after the opening quote action leaves view and hide it when it would collide with the footer. The global Chat on WhatsApp control remains above it.
