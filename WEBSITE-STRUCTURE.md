...
The Advertorial is the conversion-focused editorial format. Publishing requires one published Primary Tour. Its opening connects the visitor's planning situation to concrete, verified itinerary and package facts without fabricated urgency or proof. Directly below the excerpt, render a fully expanded **What we’ll cover** outline containing every body H2 and H3, with H3 links nested beneath the preceding H2. Every target heading receives a deterministic unique ID. The outline is part of the article in normal flow: simple anchors only, with no sticky behaviour or current-section highlighting. Place the early **Request a quote** action after the outline, repeat it only at meaningful decision points, and send every instance into the existing intake, private recovery, message-review, and visitor-controlled WhatsApp/email handoff.

On desktop, the Advertorial hero's breadcrumbs, public context and H1 span the full content width. Its second row pairs the excerpt, outline and opening quote action with a fixed 4:3 featured image. The main article uses a 680–720px reading column with an approximately 320px sticky Primary Tour panel. Mobile hides the hero image and reveals a sticky quote action only after the opening CTA leaves view; it must respect safe areas and stack below the global Chat on WhatsApp control without obscuring content. The featured image remains optional. No permanent form or direct quote-to-WhatsApp jump is allowed.
...
1. Standard Page title and breadcrumb band with the only H1.
2. A single-column introduction of at most two paragraphs focused on Holiday Kenya Safaris, formed from the **Brand Script** section of the [client-supplied Google Doc](https://docs.google.com/document/d/1S2VXcHRJrdqRUWnoEQutxg2EzbWV6Q1OHwEW9vGpi8Y/edit?tab=t.ryz9ncvyyxv7), tab `t.ryz9ncvyyxv7`. Include ownership and operation by Ashford Tours & Travel and the operator's 20+ years of travel experience discreetly within this narrative. Omit a separate introductory headline and CTA.
3. A compact two-column area with **Visit Us** on the left and **Contact Us** on the right, stacking below 768px. Visit Us shows working hours first, then the address, then the smaller embedded Google Map and directions link. The confirmed location is Suite 101, Twiga Towers, Nairobi; working hours are Monday–Saturday, 8am–5pm. Retain the client-entered Map URL for directions. The separate optional embed URL defaults to the verified Twiga Towers URL from Google Maps **Share → Embed a map**; editors may replace or clear it. Render a responsive, lazy-loaded iframe with an accessible title only when that embed URL is valid, at 220px desktop and 200px mobile height. Keep spacing minimal using the About component values in `DESIGN.md`.
4. Within that same area, Contact Us shows the public phone number with a **Chat on WhatsApp** button, followed by “Keep up with Holiday Kenya Safaris updates on social media” and compact Facebook and Instagram follow buttons with descriptive labels. All three buttons use platform branding: WhatsApp Green with its recognizable logo and Midnight Navy label/icon, Facebook blue with a white `f` logo, and an Instagram purple/pink gradient with a white camera logo, as requested by the client. Read the phone, WhatsApp, and existing Social links repeater from HKS Settings. This compact About section has no email row; global contact components retain their existing behavior.
