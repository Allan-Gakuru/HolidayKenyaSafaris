...
- The homepage is the only header exception: it omits the utility bar and overlays the existing logo and primary navigation directly on the hero using an almost-clear, lightly blurred Pale Mist surface that scrolls away. Internal pages retain the two-level header.
